...
namespace _24hplusdotnetcore.Services.MC
{
    public interface IRestMCService
    {
        ...
        [Post("/mcMobileService/service/v1.0/mobile-4sales/check-init-contract")]
        [Headers("Authorization: Bearer")]
        Task<CheckInitContractRestResponse> CheckInitContractAsync(CheckInitContractRestRequest checkInitContractRestRequest);

        [Get("/mcMobileService/service/v1.0/mobile-4sales/getApprovedInfo?appNumber={appNumber}")]
        [Headers("Authorization: Bearer")]
        Task<RestApprovedInfoResponse> GetApprovedInfoAsync(int appNumber);

        [Post("/mcMobileService/service/v1.0/mobile-4sales/requestOTP3P")]
        Task<JObject> SendOtp([Body] SendOtpRestRequest sendOtpRequest, [Header("Authorization")] string token);

        [Post("/mcMobileService/service/v1.0/mobile-4sales/scoring3P")]
        Task <JObject> SendScoring3P([Body] Scoring3PRestRequest scoring3PRequest, [Header("Authorization")] string token);

        [Post("/mcMobileService/service/v1.0/mobile-4sales/requestOTP3P")]
        Task<JObject> SendOtpV2([Body] SendOtpRestRequest sendOtpRequest, [Header("Authorization")] string token);

        [Post("/mcMobileService/service/v1.0/mobile-4sales/scoring3P")]
        Task <JObject> SendScoring3PV2([Body] Scoring3PRestRequest scoring3PRequest, [Header("Authorization")] string token);

        [Post("/api/v1.0/mcportal/rule/checkRule")]
        Task<JObject> CheckRule([Body] MCAgeCheckingDto request, [Header("Authorization")] string token, [Header("x-security")] string xSecurity, [Header("x-version")] string xVersion);
    }
}
