...
{
    public class ProductCategoryServices : IScopedLifetime
    {
        ...
        public List<ProductCategory> GetProductCategories(string productLine, string greenType)
        {
            var lstProductCategories = new List<ProductCategory>();
            try
            {

                lstProductCategories = _productCategory.Find(x => x.ProductLine.Any(p => p == productLine) && x.GreenType == greenType).ToList();
            }
            catch (Exception ex)
            {
