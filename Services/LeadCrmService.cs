...
{
    public class LeadCrmService : IScopedLifetime
    {
        ...
        {
            leadCrm.Createdtime = leadCrm.Createdtime ?? DateTime.Now;
        ...
        public async Task<LeadCrm> GetByPhoneAsync(string phone)
        {
            DateTime datefrom = DateTime.Now.AddDays(-15);
            var filter = Builders<LeadCrm>.Filter.Gte(c => c.Createdtime, datefrom);
            filter &= Builders<LeadCrm>.Filter.Eq(c => c.Cf854, phone);
            return await _leadCrmCollection.Find(filter).FirstOrDefaultAsync();
        }
    }
}
