...
{
    public class MAFCDeferService : IScopedLifetime
    {
        private readonly ILogger<MAFCDeferService> _logger;
        private readonly IMapper _mapper;
        private readonly IMongoCollection<MAFCDeferModel> _collection;

        public MAFCDeferService(
            IMongoDbConnection connection,
            ILogger<MAFCDeferService> logger,
            IMapper mapper,
            DataCRMProcessingServices dataCRMProcessingServices)
        {
            ...
            _collection = database.GetCollection<MAFCDeferModel>(MongoCollection.MAFCDefer);
        }

        public bool CreateOne(MAFCDeferModel model)
        {
            try
            {
        ...
        public List<MAFCDeferModel> GetListByQuery(string textSearch, string fromDate, string toDate, int? pagenumber, int? pagesize, ref int totalPage, ref int totalrecord)
        {
            ...
            try
            {
                ...
                var filterList = Builders<MAFCDeferModel>.Filter.Gte(c => c.CreatedTime, _datefrom) & Builders<MAFCDeferModel>.Filter.Lte(c => c.CreatedTime, _dateto);

                if (!string.IsNullOrEmpty(textSearch))
                {
                    var filterSearch = Builders<MAFCDeferModel>.Filter.Or(
                        Builders<MAFCDeferModel>.Filter.Regex(c => c.Client_name, ".*" + textSearch.ToUpper() + ".*"),
                        Builders<MAFCDeferModel>.Filter.Regex(c => c.Id_f1, ".*" + textSearch + ".*"));
